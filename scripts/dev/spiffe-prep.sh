...
AUDIENCE="${LYRIC_IAM_SPIFFE_AUDIENCE:-http://127.0.0.1:8009}"
TTL="${LYRIC_IAM_SPIFFE_TTL:-24h}"
...
# LYRIC_IAM_SPIFFE_BUNDLE_JWKS as either raw JSON or base64-encoded JWKS JSON;
...
BEGIN='# <<<LYRIC_IAM_SPIFFE_DEV>>>'
END='# <<</LYRIC_IAM_SPIFFE_DEV>>>'
...
LYRIC_IAM_SPIFFE_BUNDLE_JWKS=$BUNDLE_B64
...
echo "==> .env updated with LYRIC_IAM_SPIFFE_* (enforce mode, bundle inline as env)"
echo
echo "next steps:"
