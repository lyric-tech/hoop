...
func TestProtectionProfileLifecycleSmoke(t *testing.T) {
	...
	assertCount(t, 7, `SELECT COUNT(*) FROM private.guardrail_rules WHERE org_id = ? AND managed_by = 'lyric-iam'`, orgID)
	assertCount(t, 1, `SELECT COUNT(*) FROM private.datamasking_rules WHERE org_id = ? AND managed_by = 'lyric-iam'`, orgID)
	assertCount(t, 2, `SELECT COUNT(*) FROM private.access_request_rules WHERE org_id = ? AND managed_by = 'lyric-iam'`, orgID)
	assertCount(t, 1, `SELECT COUNT(*) FROM private.ai_session_analyzer_rules WHERE org_id = ? AND managed_by = 'lyric-iam'`, orgID)
	...
	for _, table := range []string{"attributes", "guardrail_rules", "datamasking_rules", "access_request_rules", "ai_session_analyzer_rules"} {
		assertCount(t, 0, `SELECT COUNT(*) FROM private.`+table+` WHERE org_id = ? AND managed_by = 'lyric-iam'`, orgID)
	}
	var profile *string
