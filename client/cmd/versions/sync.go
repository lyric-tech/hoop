...
var syncCmd = &cobra.Command{
	Use:   "sync",
	Short: "Install the lyric-iam CLI version reported by the connected gateway",
	Long: `Ask the connected lyric-iam gateway for its version, download the matching
release for this OS/arch, and switch the active lyric-iam CLI to it.
...
release instead, use ` + "`lyric-iam versions upgrade`" + ` (which doesn't
...
func runSync() error {
	...
	if err != nil {
		return fmt.Errorf("%v; run `lyric-iam login` first", err)
	}
...
// be clear about *why* `lyric-iam versions sync` cannot continue and *what*
...
This usually means you're connected to a local development build that wasn't stamped with a release tag.
  ...
  - To install the latest published release instead, run `+"`lyric-iam versions upgrade`"+`.`,
			apiURL, target)

	case errors.Is(err, upgrade.ErrBelowWindowsFloor):
		winFloor := upgrade.MinInstallableVersionWindows[1:] // strip leading "v"
		return fmt.Errorf(`the gateway at %s is on version %s, but `+"`lyric-iam versions`"+` only supports Windows from %s onward.
Lyric IAM CLIs older than %s don't manage themselves correctly on Windows, so a matching CLI can't be installed for you here.

What to do:
  - recommended: upgrade the gateway to %s or newer, then re-run `+"`lyric-iam versions sync`"+`.
  - manual: download a Windows build of lyric-iam %s by hand from https://github.com/hoophq/hoop/releases. That CLI won't be able to self-manage on Windows, so future version changes will also need to be done by hand.`,
			apiURL, target, winFloor, winFloor, winFloor, target)

	case errors.Is(err, upgrade.ErrBelowFloor):
		return fmt.Errorf(`the gateway at %s is on version %s, but the `+"`lyric-iam versions sync`"+` command was only introduced in %s.0.
Lyric IAM CLIs older than %s.0 don't ship this command, so it can't install a matching CLI for you.

What to do:
  - recommended: upgrade the gateway to %s.0 or newer, then re-run `+"`lyric-iam versions sync`"+`.
  - manual: download lyric-iam %s by hand from https://github.com/hoophq/hoop/releases (or via brew). That CLI won't have `+"`lyric-iam versions sync`"+`, so future version changes will also need to be done by hand.`,
...
This usually means the endpoint behind api_url isn't a lyric-iam gateway. Verify the URL with:
  lyric-iam config view`,
			apiURL, target)

	default:
		// Unknown error class — keep the original chain so debugging
		// is still possible without us masking the root cause.
		return fmt.Errorf("lyric-iam versions sync cannot proceed against the gateway at %s: %w", apiURL, err)
	}
}
